
export default function Faq() {
  return (
    <div>
        
       <h3 className="mt-10 font-bold text-gray-400 mb-5">  Frequently asked questions</h3>

        <div className="mt-5">
            <h1 className="text-gray-600">What is the best way to start trading</h1>
            <p>
                Lorem ipsum dolor sit amet consectetur adipisicing elit. Odio ex deserunt saepe a nulla, voluptate architecto excepturi vitae facere voluptas soluta maxime natus molestiae earum ullam, modi qui fuga placeat!
            </p>
        </div>
        <div className="mt-5">
            <h1 className="text-gray-600">How much money do I need to start with</h1>
            <p>
                Lorem ipsum dolor sit amet consectetur adipisicing elit. Odio ex deserunt saepe a nulla, voluptate architecto excepturi vitae facere voluptas soluta maxime natus molestiae earum ullam, modi qui fuga placeat!
            </p>
        </div>
        <div className="mt-5">
            <h1 className="text-gray-600">Where can I find the best resources</h1>
            <p>
                Lorem ipsum dolor sit amet consectetur adipisicing elit. Odio ex deserunt saepe a nulla, voluptate architecto excepturi vitae facere voluptas soluta maxime natus molestiae earum ullam, modi qui fuga placeat!
            </p>
        </div>
        <div className="mt-5">
            <h1 className="text-gray-600">I want to read charts, where do I start?</h1>
            <p>
                Lorem ipsum dolor sit amet consectetur adipisicing elit. Odio ex deserunt saepe a nulla, voluptate architecto excepturi vitae facere voluptas soluta maxime natus molestiae earum ullam, modi qui fuga placeat!
            </p>
        </div>
        <div className="mt-5">
            <h1 className="text-gray-600">Where should I begin trading</h1>
            <p>
                Lorem ipsum dolor sit amet consectetur adipisicing elit. Odio ex deserunt saepe a nulla, voluptate architecto excepturi vitae facere voluptas soluta maxime natus molestiae earum ullam, modi qui fuga placeat!
            </p>
        </div>
      
        </div>
  )
}
